...
2. **libmpv / FFmpeg / libplacebo**：同一 DevShell + **VS `VC\Tools\Llvm\x64` 的 clang / lld-link**（须让 VS LLVM 排在独立 `C:\Program Files\LLVM` 之前）
...
5. 必须 `-Dffmpeg:vvc_decoder=disabled`（meson-ports `meson-8.1` 在 aarch64 脚本里误引用 `libavcodec_x86_optional_sources`）
